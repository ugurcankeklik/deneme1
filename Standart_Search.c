...
{  
    ...
    else{ // If frontier is not empty, find appropriate element according to the ordered evaluation function values. 
        if(child->path_cost + alpha * child->state.h_n < (*frontier)->node->path_cost + alpha * (*frontier)->node->state.h_n) { // Child has the lowest cost evaluation function value
            new_queue->next = *frontier;
            *frontier = new_queue; 
        }
        else{
